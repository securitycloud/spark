...
# old way to start slave: $WRK/spark-bin-hadoop/bin/spark-class org.apache.spark.deploy.worker.Worker $MASTERURL > /dev/null 2>&1 &
...
do
	...
	then
		ssh root@${i} "
			...
			$WRK/spark-bin-hadoop/sbin/start-slave.sh $MASTERURL
		"
